...
Try {
	...
	[string]$appName = 'Acrobat Pro (Unified Installer)'
	[string]$appVersion = 'Latest'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.1.0'
	[string]$appScriptDate = '13/11/2025'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Remove-File -Path "$envCommonDesktop\Adobe*.lnk" -ErrorAction SilentlyContinue

		## Configure settings for Adobe Acrobat
		$SoftwareRegKey = "HKLM:\SOFTWARE"

		#Disable EULA
		Set-RegistryKey -Key "$($SoftwareRegKey)\Adobe\Adobe Acrobat\DC\AdobeViewer" -Name "EULA" -Value "1" -Type "DWord"
		#Dsable the prompt "Make Adobe Acrobat my default PDF application."
		Set-RegistryKey -Key "$($SoftwareRegKey)\Adobe\Adobe Acrobat\DC\AVAlert\cCheckbox" -Name "iAppDoNotTakePDFOwnershipAtLaunch" -Value "1" -Type "DWord"
		Set-RegistryKey -Key "$($SoftwareRegKey)\Adobe\Adobe Acrobat\DC\AVAlert\cCheckbox" -Name "iAppDoNotTakePDFOwnershipAtLaunchWin10" -Value "1" -Type "DWord"
		#Disable the Help > Repair Installation menu
		Set-RegistryKey -Key "$($SoftwareRegKey)\Adobe\Adobe Acrobat\DC\Installer" -Name "DisableMaintenance" -Value "1" -Type "DWord"
		#Disable messages which encourage the user to upgrade the product
		Set-RegistryKey -Key "$($SoftwareRegKey)\Policies\Adobe\Adobe Acrobat\DC\FeatureLockDown" -Name "bAcroSuppressUpsell" -Value "1" -Type "DWord"
		#Disable the First Time Experience (FTE) feature (Welcome tour/page)
		Set-RegistryKey -Key "$($SoftwareRegKey)\Policies\Adobe\Adobe Acrobat\DC\FeatureLockDown" -Name "bToggleFTE" -Value "1" -Type "DWord"
		#Disable user participation in the feedback program
		Set-RegistryKey -Key "$($SoftwareRegKey)\Policies\Adobe\Adobe Acrobat\DC\FeatureLockDown" -Name "bUsageMeasurement" -Value "0" -Type "DWord"
		...
		Set-RegistryKey -Key "$($SoftwareRegKey)\Policies\Adobe\Adobe Acrobat\DC\FeatureLockDown" -Name "bIsSCReducedModeEnforcedEx" -Value "1" -Type "DWord"
		Set-RegistryKey -Key "$($SoftwareRegKey)\Policies\Adobe\Adobe Acrobat\DC\FeatureLockDown\cIPM" -Name "bDontShowMsgWhenViewingDoc" -Value "0" -Type "DWord"
