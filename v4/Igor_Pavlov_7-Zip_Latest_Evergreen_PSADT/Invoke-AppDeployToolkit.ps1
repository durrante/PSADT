...
$adtSession = @{
    ...
    AppProcessesToClose = @(@{ Name = '7zFM'; Description = '7-Zip File Manager' }, @{ Name = '7zG'; Description = '7-Zip GUI' }, @{ Name = '7z'; Description = '7-Zip Command Line' }, @{ Name = '7za'; Description = '7-Zip Command Line' })  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    # For ZIP installs, leave $appZipInstallerPath empty to auto-detect a likely installer, or set it manually.

    $appEvergreenName = '7zip'
    $appArchitecture = 'x64'
    $appType = 'msi'
    ...
    $appZipInstallerPath = ''  # Example: 'Adobe Acrobat\setup.exe'
    ...
    switch ($installerExtension)
    {
        ...
        '.zip'
        {
            ...
            # The selected Evergreen package currently resolves to a ZIP download (msi).
            # Example manual inner path: 'Adobe Acrobat\setup.exe'
            if ([string]::IsNullOrWhiteSpace($appZipInstallerPath))
            {
                $zipInstallerCandidates = @(
                    Get-ChildItem -LiteralPath $extractPath -Recurse -File -Include '*.msi', 'setup.exe', 'install.exe' |
                        Sort-Object -Property FullName -Unique
                )

                if ($zipInstallerCandidates.Count -eq 1)
                {
                    $expandedInstallerPath = $zipInstallerCandidates[0].FullName
                }
                else
                {
                    $preferredSetupCandidates = @($zipInstallerCandidates | Where-Object { $_.Name -ieq 'setup.exe' })
                    if ($preferredSetupCandidates.Count -eq 1)
                    {
                        $expandedInstallerPath = $preferredSetupCandidates[0].FullName
                    }
                    else
                    {
                        $candidateList = if ($zipInstallerCandidates.Count -gt 0)
                        {
                            ($zipInstallerCandidates | ForEach-Object {
                                if ($_.FullName.StartsWith($extractPath, [System.StringComparison]::OrdinalIgnoreCase))
                                {
                                    $_.FullName.Substring($extractPath.Length).TrimStart('\')
                                }
                                else
                                {
                                    $_.FullName
                        ...
                        throw "Unable to auto-detect a single installer inside the extracted ZIP. Set [$appZipInstallerPath] manually. Candidates: $candidateList"
                    }
                }
            }
            else
            ...
            {
                throw "Specified installer file was not found inside the extracted ZIP: [$expandedInstallerPath]."
            }
    ...
    $appDesktopIconPattern = '7-Zip*.lnk'
