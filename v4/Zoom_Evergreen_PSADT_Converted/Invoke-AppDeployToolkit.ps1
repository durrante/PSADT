...
$adtSession = @{
    ...
    AppProcessesToClose = 'zoom'  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
		...
		$appInfo = Get-EvergreenApp -Name $adtSession.AppName | Where-Object { $_.Architecture -eq $adtSession.AppArch -and $_.Installer -eq $appInstaller -and $_.Platform -eq $appPlatform}  | `
		Sort-Object -Property @{ Expression = { [System.Version]$_.Version }; Descending = $true } | Select-Object -First 1		
		$installerPath = $appInfo | Save-EvergreenApp -Path $tempPath

		# Install Zoom
		Start-ADTMsiProcess -Action 'Install' -FilePath "$installerPath" -ArgumentList '/qn'
		...
		## Remove desktop sh
		Sleep -Seconds 5
		Remove-ADTFile -Path "$envCommonDesktop\Zoom Workplace.lnk" -ErrorAction 0
...
function Uninstall-ADTDeployment
{
		...
		Show-ADTInstallationWelcome -CloseProcesses 'zoom' -AllowDeferCloseProcesses -DeferTimes $MaxDefers -ForceCloseProcessesCountdown $CloseAppsCountDownSeconds -PromptToSave -PersistPrompt
		...
		Uninstall-ADTApplication -Name 'Zoom Workplace' -ApplicationType MSI
