...
$adtSession = @{
    ...
    AppProcessesToClose = @(@{ Name = 'obs32'; Description = 'OBS Studio' }, @{ Name = 'obs64'; Description = 'OBS Studio' })  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    ## Remove any existing OBS Studio installation before upgrading.
    $existingOBSStudio = @(Get-ADTApplication -Name 'OBS Studio' -NameMatch Contains -ErrorAction SilentlyContinue)
    if ($existingOBSStudio.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingOBSStudio.Count) existing OBS Studio installation(s) - removing before upgrade."
        $preUninstallSplat = @{
            Name                   = 'OBS Studio'
            NameMatch              = 'Contains'
            ApplicationType        = 'EXE'
    ...
    $appInfo = Get-EvergreenApp -Name $appEvergreenName | Where-Object { $_.Architecture -eq $appArchitecture } |
        Sort-Object -Property @{ Expression = { [version]$_.Version }; Descending = $true } |
        Select-Object -First 1
    if ($null -eq $appInfo)
    {
