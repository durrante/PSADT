...
function Install-ADTDeployment
{
    ...
    ## Remove any existing PuTTY installation before upgrading.
    $existingPuTTY = @(Get-ADTApplication -Name 'PuTTY' -NameMatch Contains -ErrorAction SilentlyContinue)
    if ($existingPuTTY.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingPuTTY.Count) existing PuTTY installation(s) - removing before upgrade."
        $preUninstallSplat = @{
            Name                   = 'PuTTY'
            NameMatch              = 'Contains'
            ApplicationType        = 'EXE'
    ...
    $appDesktopIconPattern = 'PuTTY*.lnk'
