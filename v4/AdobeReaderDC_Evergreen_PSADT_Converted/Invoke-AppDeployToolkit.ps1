...
$adtSession = @{
    ...
    AppProcessesToClose = 'Acrobat'  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
		...
		Show-ADTInstallationWelcome -CloseProcesses 'Acrobat' -CheckDiskSpace -PersistPrompt -CloseProcessesCountdown 600
		...
		$adtSession.AppName = "AdobeAcrobatReaderDC"
		$adtSession.AppLang = "English (UK)"
		...
		$appInfo = Get-EvergreenApp -Name $adtSession.AppName | Where-Object { $_.Architecture -eq $adtSession.AppArch -and $_.Type -eq $appType -and $_.Language -eq $adtSession.AppLang}  | `
		Sort-Object -Property @{ Expression = { [System.Version]$_.Version }; Descending = $true } | Select-Object -First 1		
		$installerPath = $appInfo | Save-EvergreenApp -Path $tempPath

		# Install cmd
        Start-ADTProcess -FilePath "$installerPath" -ArgumentList "/sAll /msi /norestart /quiet ALLUSERS=1 EULA_ACCEPT=YES" -WindowStyle Hidden -WaitForMsiExec
		...
		Remove-ADTFile -Path "$envCommonDesktop\Adobe Acrobat*.lnk" -ErrorAction SilentlyContinue
		...
		#Disable EULA
		Set-ADTRegistryKey -LiteralPath "$($SoftwareRegKey)\Adobe\Acrobat Reader\DC\AdobeViewer" -Name "EULA" -Value "1" -Type "DWord"
		#Dsable the prompt "Make Adobe Acrobat my default PDF application."
		Set-ADTRegistryKey -LiteralPath "$($SoftwareRegKey)\Adobe\Acrobat Reader\DC\AVAlert\cCheckbox" -Name "iAppDoNotTakePDFOwnershipAtLaunch" -Value "1" -Type "DWord"
		Set-ADTRegistryKey -LiteralPath "$($SoftwareRegKey)\Adobe\Acrobat Reader\DC\AVAlert\cCheckbox" -Name "iAppDoNotTakePDFOwnershipAtLaunchWin10" -Value "1" -Type "DWord"
		#Disable the Help > Repair Installation menu
		Set-ADTRegistryKey -LiteralPath "$($SoftwareRegKey)\Adobe\Acrobat Reader\DC\Installer" -Name "DisableMaintenance" -Value "1" -Type "DWord"
		#Disable messages which encourage the user to upgrade the product
		Set-ADTRegistryKey -LiteralPath "$($SoftwareRegKey)\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown" -Name "bAcroSuppressUpsell" -Value "1" -Type "DWord"
		#Disable the First Time Experience (FTE) feature (Welcome tour/page)
		Set-ADTRegistryKey -LiteralPath "$($SoftwareRegKey)\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown" -Name "bToggleFTE" -Value "1" -Type "DWord"
		#Disable user participation in the feedback program
		Set-ADTRegistryKey -LiteralPath "$($SoftwareRegKey)\Policies\Adobe\Acrobat Reader\DC\FeatureLockDown" -Name "bUsageMeasurement" -Value "0" -Type "DWord"
...
function Uninstall-ADTDeployment
{
		...
		Show-ADTInstallationWelcome -CloseProcesses 'Acrobat' -CloseProcessesCountdown 600
		...
		Uninstall-ADTApplication -Name 'Adobe Acrobat' -ApplicationType MSI
