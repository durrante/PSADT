...
function Install-ADTDeployment
{
    ...
    ## Install via WinGet.
    ## NOTE: previously installed via Evergreen, which downloads from SourceForge. That failed in this
    ## environment with "You cannot call a method on a null-valued expression" inside Evergreen's
    ## Resolve-SystemNetWebRequest - a bug in Evergreen's own finally block that masks the real cause
    ## (a failed GetResponse(), most likely sourceforge.net being unreachable/blocked under the SYSTEM
    ## context). Switched to WinGet to avoid SourceForge entirely.
...
function Repair-ADTDeployment
{
    ...
    Repair-ADTWinGetPackage -Id 'WinSCP.WinSCP'
