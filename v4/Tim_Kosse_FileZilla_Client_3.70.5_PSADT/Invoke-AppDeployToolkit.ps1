...
$adtSession = @{
    ...
    AppProcessesToClose = @(@{ Name = 'filezilla'; Description = 'FileZilla Client' })  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    ## Remove any existing FileZilla Client installation before upgrading.
    $existingFileZillaClient = @(Get-ADTApplication -Name 'FileZilla Client' -NameMatch Contains -ErrorAction SilentlyContinue)
    if ($existingFileZillaClient.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingFileZillaClient.Count) existing FileZilla Client installation(s) - removing before upgrade."
        $preUninstallSplat = @{
            Name                   = 'FileZilla Client'
            NameMatch              = 'Contains'
            ApplicationType        = 'EXE'
            AdditionalArgumentList = '/S'
        }
    ...
    $appInstallArgumentList = @(
        # Add one EXE argument per line.
        # Examples: /S, /quiet, /norestart
        '/S'
    )
    $installProcessSplat = @{
        FilePath       = "$($adtSession.DirFiles)\FileZilla_3.70.5_win64-setup.exe"
        WindowStyle    = 'Hidden'
        WaitForMsiExec = $true
    }
    if ($appInstallArgumentList.Count -gt 0)
    {
        $installProcessSplat.ArgumentList = $appInstallArgumentList
    }
    Start-ADTProcess @installProcessSplat
    ...
    ## Write fzdefaults.xml to the FileZilla installation directory.
    ## This disables the update check and sets the config location.
    $fileZillaApp = Get-ADTApplication -FilterScript { $_.DisplayName -match '^FileZilla(?!Server)(?!\s+Server).*$' }
    if ($fileZillaApp)
    {
        $fzDefaultsPath = Join-Path -Path $fileZillaApp.InstallLocation -ChildPath 'fzdefaults.xml'
        Write-ADTLogEntry -Message "Writing fzdefaults.xml to '$fzDefaultsPath'." -Severity 1
        $fzDefaultsContent = @(
            '<FileZilla3>'
            '  <Settings>'
        ...
        [System.IO.File]::WriteAllText($fzDefaultsPath, $fzDefaultsContent, [System.Text.UTF8Encoding]::new($false))
    }
    else
    {
        Write-ADTLogEntry -Message "FileZilla Client installation not found - fzdefaults.xml was not written." -Severity 2
    }

    ## Adjust the pattern below if the application creates a differently named shortcut on the desktop.
    $appDesktopIconPattern = 'FileZilla*.lnk'
...
function Uninstall-ADTDeployment
{
    ...
    ## Uninstall FileZilla Client, explicitly excluding Server editions.
    $uninstallSplat = @{
        FilterScript           = { $_.DisplayName -match '^FileZilla(?!Server)(?!\s+Server).*$' }
        ApplicationType        = 'EXE'
        AdditionalArgumentList = '/S'
    }
