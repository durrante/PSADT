...
$adtSession = @{
    ...
    AppProcessesToClose = @(@{ Name = 'googleearth'; Description = 'Google Earth Pro' })  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    ## Remove any existing Google Earth installation before upgrading.
    $existingGoogleEarth = @(Get-ADTApplication -Name 'Google Earth Pro' -NameMatch Contains -ErrorAction SilentlyContinue)
    if ($existingGoogleEarth.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingGoogleEarth.Count) existing Google Earth installation(s) - removing before upgrade."
        $preUninstallSplat = @{
    ...
    ## Ensure WinGet is operational before deploying.
    ...
    $winGetInstallSplat = @{
        ...
        Scope        = 'System'
        Architecture = 'X64'
        Custom       = 'DESKTOPSHORTCUT=0'
    }
    Install-ADTWinGetPackage @winGetInstallSplat
    ...
    $appDesktopIconPattern = 'Google Earth*.lnk'
...
function Uninstall-ADTDeployment
{
    ...
    ## Do Not Show Google Earth Pro Start-up Tips & Do Not Send Google Earth Pro Anonymous Usage Statistics
    Invoke-ADTAllUsersRegistryAction {
        Set-ADTRegistryKey -Key 'HKCU\Software\Google\Google Earth Pro' -Name 'enableTips' -Value 'false' -Type String -SID $_.SID
        Set-ADTRegistryKey -Key 'HKCU\Software\Google\Google Earth Pro' -Name 'UsageStats2' -Value 'false' -Type String -SID $_.SID
    }    
}
...
function Repair-ADTDeployment
{
    ...
    ## Repair the application via WinGet.
    Repair-ADTWinGetPackage -Id 'Google.EarthPro'
