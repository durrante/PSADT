...
$adtSession = @{
    ...
    AppProcessesToClose = @(@{ Name = 'Chrome'; Description = 'Google Chrome' })  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    foreach ($user in (Get-ChildItem -Path 'C:\Users')) {
        ...
        if (Test-Path -LiteralPath $GChromeLocal) {
            $UninstPath = Get-ChildItem -Path "$GChromeLocal\*" -Include 'setup.exe' -Recurse -ErrorAction SilentlyContinue |
                Select-Object -First 1
            if ($null -ne $UninstPath -and $UninstPath.Exists) {
                Write-ADTLogEntry -Message "Found $($UninstPath.FullName), attempting to uninstall $($adtSession.AppName)."
                Start-ADTProcessAsUser -FilePath $UninstPath.FullName -ArgumentList '--uninstall', '--force-uninstall' -IgnoreExitCodes '19', '20'
                Start-Sleep -Seconds 5
                ## Cleanup User Profile Registry
                Invoke-ADTAllUsersRegistryAction -ScriptBlock {
                    Remove-ADTRegistryKey -Key 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\Google Chrome'
                } -ErrorAction SilentlyContinue
                ## Cleanup Google Chrome Application (Local User Profile) Directory
                if (Test-Path -LiteralPath $GChromeLocal) {
                    Write-ADTLogEntry -Message "Cleaning up directory: $GChromeLocal"
    ...
    ## Remove Google Chrome Start Menu and Desktop shortcuts from all user profiles
    foreach ($user in (Get-ChildItem -Path 'C:\Users')) {
        $StartMenuSC = "$($user.FullName)\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Google Chrome"
        if (Test-Path -LiteralPath $StartMenuSC) {
            Remove-Item -Path $StartMenuSC -Recurse -Force -ErrorAction SilentlyContinue
        }
        $DesktopSC = "$($user.FullName)\Desktop\Google Chrome.lnk"
        if (Test-Path -LiteralPath $DesktopSC) {
            Remove-Item -Path $DesktopSC -Force -ErrorAction SilentlyContinue
        }
    }

    ## Uninstall any existing system-level versions of Google Chrome
    $GChromeInstalled = Get-ADTApplication -Name 'Google Chrome' | Select-Object -ExpandProperty UninstallString
    if (@($GChromeInstalled).Count -gt 1) {
        foreach ($item in $GChromeInstalled) {
            if ($item -match 'setup\.exe') {
                $UninstallString = $item.Split('"')[1]
                Start-ADTProcess -FilePath $UninstallString -ArgumentList '--uninstall', '--system-level', '--multi-install', '--force-uninstall' -WindowStyle Hidden -IgnoreExitCodes '20'
            }
            elseif ($item -match '\{') {
    ...
    ## Uninstall Google Update Helper (Google-specific only, to avoid affecting other Chromium browsers)
    $GUpdateHelper = Get-ADTApplication -Name 'Google Update Helper' |
        Where-Object { $_.InstallSource -match 'Google' } |
    ...
    if ($GUpdateHelper -match 'GoogleUpdate\.exe') {
        if (Test-Path -Path "$envProgramFiles\Google\Update\GoogleUpdate.exe") {
            Start-ADTProcess -FilePath "$envProgramFiles\Google\Update\GoogleUpdate.exe" -ArgumentList '/uninstall'
        }
        elseif (Test-Path -Path "$envProgramFilesX86\Google\Update\GoogleUpdate.exe") {
            Start-ADTProcess -FilePath "$envProgramFilesX86\Google\Update\GoogleUpdate.exe" -ArgumentList '/uninstall'
        }
    }
    elseif ($GUpdateHelper -match '\{') {
        $GUpdateHelperProductCode = ($GUpdateHelper -replace 'msiexec\.exe', '' -replace '/I', '' -replace '/X', '').Trim()
        Start-ADTMsiProcess -Action Uninstall -FilePath $GUpdateHelperProductCode -ArgumentList '/QN', 'REBOOT=ReallySuppress'
    }
    ## Fail-safe: remove GoogleUpdate.exe if still present
    if (Test-Path -Path "$envProgramFiles\Google\Update\GoogleUpdate.exe") {
        Start-ADTProcess -FilePath "$envProgramFiles\Google\Update\GoogleUpdate.exe" -ArgumentList '/uninstall'
    }
    elseif (Test-Path -Path "$envProgramFilesX86\Google\Update\GoogleUpdate.exe") {
    ...
    $appInfo = Get-EvergreenApp -Name $appEvergreenName | Where-Object { $_.Architecture -eq $appArchitecture -and $_.Type -eq $appType -and $_.Channel -eq $appChannel } |
        Sort-Object -Property @{ Expression = { [version]$_.Version }; Descending = $true } |
        Select-Object -First 1
    if ($null -eq $appInfo)
    {
    ...
    $appDesktopIconPattern = 'Google*.lnk'
    ...
    ## Prevent creation of the Desktop shortcut via initial_preferences
    $chromeOpenCmd = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Classes\ChromeHTML\shell\open\command' -Name '(Default)' -ErrorAction SilentlyContinue).'(Default)'
    if ($chromeOpenCmd) {
        $chromeInstallLocation = Split-Path ($chromeOpenCmd.Split('--')[0].Trim('"', ' ')) -Parent
        $prefFile = Join-Path -Path $chromeInstallLocation -ChildPath 'initial_preferences'
        if (Test-Path -Path $prefFile -PathType Leaf) {
            try {
                $json = Get-Content -Path $prefFile -Raw | ConvertFrom-Json
                if ($json) {
                    if ($null -ne $json.distribution) {
