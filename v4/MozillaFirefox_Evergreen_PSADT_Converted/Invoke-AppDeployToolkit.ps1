...
function Install-ADTDeployment
{
		...
		Show-ADTInstallationWelcome -CloseProcesses 'firefox' -CheckDiskSpace -PersistPrompt -CloseProcessesCountdown 600
		...
		$adtSession.AppName = "MozillaFirefox"
		$appArchitecture = "x64"
		$appChannel = "current"
		$appType = "msi"
		...
		$appInfo = Get-EvergreenApp -Name $adtSession.AppName | Where-Object { $_.Architecture -eq $appArchitecture -and $_.Channel -eq $appChannel -and $_.Type -eq $appType } | `
		...
		Remove-ADTFile -Path "$envCommonDesktop\Firefox.lnk" -ErrorAction SilentlyContinue
...
function Uninstall-ADTDeployment
{
		...
		Show-ADTInstallationWelcome -CloseProcesses 'firefox' -CloseProcessesCountdown 600
		...
		$AppList = Get-ADTApplication -Name 'Mozilla Firefox'
		...
		ForEach ($App in $AppList)
		{
		  ...
		  If($App.UninstallString)
		  {
			...
			If(Test-Path -Path $UninstPath)
			{
			  ...
			  Start-ADTProcess -FilePath $UninstPath -ArgumentList '/VERYSILENT /SUPPRESSMSGBOXES /NORESTART'
			}
		  }
		}
