...
$adtSession = @{
    ...
    AppProcessesToClose = 'ShareX'  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
    ...
    InstallName = 'ShareX Installation'
    InstallTitle = 'ShareX'
...
function Install-ADTDeployment
{
		...
		## Show Welcome Message, close ShareX if required after 10 minute countdown, verify there is enough disk space to complete the install, and persist the prompt
		Show-ADTInstallationWelcome -CloseProcesses 'ShareX' -CheckDiskSpace -PersistPrompt -CloseProcessesCountdown 600
		...
		[string]$MyArguments = "-ExecutionPolicy Bypass -NoLogo -File `"$($adtSession.DirFiles)\InstallShareX.ps1`" "
...
function Uninstall-ADTDeployment
{
		...
		## Show Welcome Message, close ShareX with a 600 second countdown before automatically closing
		Show-ADTInstallationWelcome -CloseProcesses 'ShareX' -CloseProcessesCountdown 600
		...
		# Get a list of applications matching "ShareX"
		$AppList = Get-ADTApplication -Name 'ShareX'
		...
		ForEach ($App in $AppList)
		{
		  ...
		  If($App.UninstallString)
		  {
			...
			If(Test-Path -Path $UninstPath)
			{
			  ...
			  Write-ADTLogEntry -Message "Found $($App.DisplayName) ($($App.DisplayVersion)) and valid uninstall string, trying to uninstall."
