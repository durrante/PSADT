...
$adtSession = @{
    ...
    AppProcessesToClose = @(@{ Name = 'vlc'; Description = 'VLC Media Player' })  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    ## Remove any existing VLC Media Player installation before upgrading.
    $existingVLCMediaPlayer = @(Get-ADTApplication -Name 'VLC Media Player' -NameMatch Exact -ErrorAction SilentlyContinue)
    if ($existingVLCMediaPlayer.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingVLCMediaPlayer.Count) existing VLC Media Player installation(s) - removing before upgrade."
        $preUninstallSplat = @{
    ...
    $appEvergreenName = 'VideoLanVlcPlayer'
    $appArchitecture = 'x64'
    $appType = 'msi'
    ...
    $appDesktopIconPattern = 'VLC media player*.lnk'
