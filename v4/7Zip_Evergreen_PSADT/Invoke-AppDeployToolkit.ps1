...
$adtSession = @{
    ...
    AppProcessesToClose = '7z', '7zFM'  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
		...
		Show-ADTInstallationWelcome -CloseProcesses '7z', '7zFM' -CheckDiskSpace -PersistPrompt -CloseProcessesCountdown 600
        ...
        $AppList = Get-ADTApplication -Name '7-Zip'        
		...
		if (Test-Path -Path $regPath) {
			...
			if ($currentValue -eq $expectedValue) {
				Write-ADTLogEntry -Message "Registry value 'DisplayName' with the expected value found. Deleting the registry key..."
				
				# Remove the registry key and all its subkeys
				Remove-Item -Path $regPath -Recurse -Force

				if (-not (Test-Path -Path $regPath)) {
					Write-ADTLogEntry -Message "Registry key and its subkeys successfully deleted."
				} else {
					Write-ADTLogEntry -Message "Failed to delete the registry key."
				}
			} else {
				Write-ADTLogEntry -Message "Registry value 'DisplayName' does not match the expected value. No action taken."
			}
		} else {
			Write-ADTLogEntry -Message "Registry key does not exist. No action taken."
		}
		...
		$appInfo = Get-EvergreenApp -Name $appname | Where-Object { $_.Architecture -eq $adtSession.AppArch -and $_.Type -eq $appType}  | `
		Sort-Object -Property @{ Expression = { [System.Version]$_.Version }; Descending = $true } | Select-Object -First 1		
		$installerPath = $appInfo | Save-EvergreenApp -Path $tempPath

		# Install
		Start-ADTMsiProcess -Action 'Install' -FilePath "$installerPath" -ArgumentList '/qn /norestart REBOOT="ReallySuppress"'
...
function Uninstall-ADTDeployment
{
		...
		Show-ADTInstallationWelcome -CloseProcesses '7z', '7zFM' -AllowDeferCloseProcesses -DeferTimes $MaxDefers -ForceCloseProcessesCountdown $CloseAppsCountDownSeconds -PromptToSave -PersistPrompt
