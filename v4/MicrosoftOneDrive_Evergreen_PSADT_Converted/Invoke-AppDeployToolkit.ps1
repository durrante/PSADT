...
$adtSession = @{
    ...
    InstallName = ''
    InstallTitle = 'Microsoft OneDrive'
...
function Install-ADTDeployment
{
        ...
        {
        Write-ADTLogEntry -Message "Found $($UninstPath1.FullName), now attempting to uninstall $($adtSession.InstallTitle)."
        Start-ADTProcessAsUser -Path "$UninstPath1" -Parameters "/uninstall" -NoWait
        Sleep -Seconds 5

        ## Cleanup User Profile Registry
        [scriptblock]$HKCURegistrySettings = {
        Remove-ADTRegistryKey -Key 'HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\OneDriveSetup.exe' -SID $_.SID
        }
        ...
        If (Test-Path $OneDrive) {
        Write-ADTLogEntry -Message "Cleanup ($OneDrive) Directory."
        ...
        {
        Write-ADTLogEntry -Message "Found $($UninstPath2.FullName), now attempting to uninstall $($adtSession.InstallTitle)."
        Start-ADTProcess -FilePath "$UninstPath2" -ArgumentList "/uninstall /allusers" -WindowStyle Hidden
        Sleep -Seconds 5
        ...
        Write-ADTLogEntry -Message "Cleanup OneDrive (Program Files) Directory."
        ...
        {
        Write-ADTLogEntry -Message "Found $($UninstPath3.FullName), now attempting to uninstall $($adtSession.InstallTitle)."
        Start-ADTProcess -FilePath "$UninstPath3" -ArgumentList "/uninstall /allusers" -WindowStyle Hidden
        Sleep -Seconds 5
        ...
        Write-ADTLogEntry -Message "Cleanup OneDrive (Program Files x86) Directory."
        ...
        Write-ADTLogEntry -Message "Cleanup OneDrive (ProgramData) Directory."
		...
		$adtSession.AppName = "MicrosoftOneDrive"
		$appType = "exe"
		$adtSession.AppArch = "x86"
		$appRing = "Production"
		$tempPath = "C:\Temp\$($adtSession.AppName)"
		
		# Download the latest stable version of the application using the Evergreen module
		$appInfo = (Get-EvergreenApp -Name $adtSession.AppName | Where-Object { $_.Ring -eq $appRing -and $_.Architecture -eq $adtSession.AppArch -and $_.Type -eq $appType}) | `
