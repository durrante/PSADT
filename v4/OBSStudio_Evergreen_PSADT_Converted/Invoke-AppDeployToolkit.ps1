...
$adtSession = @{
    ...
    AppProcessesToClose = 'obs32', 'obs64'  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
		...
		## Show Welcome Message, close OBS if required after 10 minute countdown, verify there is enough disk space to complete the install, and persist the prompt
		Show-ADTInstallationWelcome -CloseProcesses 'obs32', 'obs64' -CheckDiskSpace -PersistPrompt -CloseProcessesCountdown 600
        ...
        {
        Write-ADTLogEntry -Message "Found $($SSMS.FullName), now attempting to uninstall existing versions of $($adtSession.InstallTitle)."
        Start-ADTProcess -FilePath "$SSMS" -ArgumentList "/uninstall /quiet /norestart" -WindowStyle Hidden
		...
		## Installs OBS Studio via Evergreen
		...
		# Download Latest version of OBS Studio via Evergreen
		$OBS = Get-EvergreenApp -Name OBSStudio | Where-Object { $_.Architecture -eq "x64"}
		$Installer = $OBS | Save-EvergreenApp -Path "C:\Temp\OBS"

		# Install Command
		$ExePath = Get-ChildItem -Path "$Installer" -Include OBS-Studio*.exe -File -Recurse -ErrorAction SilentlyContinue
		If($ExePath.Exists)
		{
		Write-ADTLogEntry -Message "Found $($ExePath.FullName), now attempting to install $($adtSession.InstallTitle)."
		Start-ADTProcess -FilePath "$ExePath" -ArgumentList "/S" -WindowStyle Hidden
		Start-Sleep -Seconds 5
		}
		...
		If (Test-Path -Path "$envCommonDesktop\OBS Studio.lnk") {
			Write-ADTLogEntry -Message "Removing OBS Studio Desktop Shortcut."
...
function Uninstall-ADTDeployment
{
		...
		## Show Welcome Message, close OBS Studio with a 600 second countdown before automatically closing
		Show-ADTInstallationWelcome -CloseProcesses 'obs32', 'obs64' -CloseProcessesCountdown 600
		...
		$AppList = Get-ADTApplication -Name 'OBS Studio'        
		...
		If (Test-Path -Path "$envProgramFiles\obs-studio\") {
			Write-ADTLogEntry -Message "Cleanup OBS Studio Program Files Directory."
		...
		If (Test-Path -Path "$envProgramFilesX86\obs-studio\") {
			Write-ADTLogEntry -Message "Cleanup OBS Studio Program Files (x86) Directory."
		...
		## Remove OBS Studio Start Menu Shortcuts (If Present)
			...
			Write-ADTLogEntry -Message "Removing OBS Studio Start Menu Shortcuts."
