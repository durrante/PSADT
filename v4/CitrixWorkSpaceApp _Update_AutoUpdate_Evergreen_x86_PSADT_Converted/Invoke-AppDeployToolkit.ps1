...
function Install-ADTDeployment
{
		...
		if ($forceSilentInstallation) {
			Show-ADTInstallationWelcome -CloseProcesses 'SelfService', 'Receiver', 'AuthManSvr', 'CtxWebBrowser', 'concentr', 'redirector', 'SelfServicePlugin', 'wfcrun32', 'wfica32' -BlockExecution -Silent -PromptToSave
		}
		else {
			# ask to close processes if required, allow up to 3 deferrals
			Show-ADTInstallationWelcome -CloseProcesses 'SelfService', 'Receiver', 'AuthManSvr', 'CtxWebBrowser', 'concentr', 'redirector', 'SelfServicePlugin', 'wfcrun32', 'wfica32' -AllowDeferCloseProcesses -DeferTimes $Defertimes -ForceCloseProcessesCountdown $CloseAppsCountDownSeconds -PromptToSave -MinimizeWindows
		}
		...
		try {
		  ...
		  if ($storeApp) {
			  ...
			  $Profiles = (Get-ADTUserProfiles).ProfilePath 
			  ForEach ($Profile in $Profiles){
				  if (Test-Path "$($Profile)\Desktop\Citrix Workspace.lnk") {
					  Remove-ADTFile -Path "$($Profile)\Desktop\Citrix Workspace.lnk"
				  }
			  }
		  }
		} catch {}
		...
		$adtSession.AppName = "CitrixWorkspaceApp"
		$appStream = "Current"
		$tempPath = "C:\Temp\$($adtSession.AppName)"
		
		# Download the latest stable version of the application using the Evergreen module
		$appInfo = Get-EvergreenApp -Name $adtSession.AppName | Where-Object { $_.Stream -eq $appStream} | `
        Sort-Object -Property @{ Expression = { [System.Version]$_.Version }; Descending = $true } | Select-Object -First 1
		$installerPath = $appInfo | Save-EvergreenApp -Path $tempPath

		# Install Citrix
		Start-ADTProcess -FilePath "$InstallerPath" -ArgumentList "/AutoUpdateCheck=Auto /AutoUpdateStream=Current /noreboot /silent EnableCEIP=false" -WindowStyle Hidden -IgnoreExitCodes 40008
		...
		## Remove desktop sh
		Sleep -Seconds 5
		Remove-ADTFile -Path "$envCommonDesktop\Citrix Workspace.lnk" -ErrorAction 0
...
function Uninstall-ADTDeployment
{
		...
		Show-ADTInstallationWelcome -CloseProcesses 'SelfService', 'Receiver', 'AuthManSvr', 'CtxWebBrowser', 'concentr', 'redirector', 'SelfServicePlugin', 'wfcrun32', 'wfica32' -AllowDeferCloseProcesses -DeferTimes $MaxDefers -ForceCloseProcessesCountdown $CloseAppsCountDownSeconds -PromptToSave -PersistPrompt
        ...
        $AppList = Get-ADTApplication -Name 'Citrix Workspace'        
        ...
        Start-ADTProcess -FilePath $UninstPath -ArgumentList '/uninstall /cleanup /silent'
