...
$adtSession = @{
    ...
    AppProcessesToClose = @(
        @{ Name = 'wireshark'; Description = 'Wireshark' }
        @{ Name = 'dumpcap'; Description = 'Dumpcap' }
    )
...
function Install-ADTDeployment
{
    ...
    ## Remove any existing Wireshark installation before upgrading.
    $existingWireshark = @(Get-ADTApplication -Name 'Wireshark' -NameMatch Exact -ErrorAction SilentlyContinue)
    if ($existingWireshark.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingWireshark.Count) existing Wireshark installation(s) - removing before upgrade."
        $preUninstallSplat = @{
            Name                   = 'Wireshark'
            NameMatch              = 'Exact'
            ApplicationType        = 'EXE'
    ...
    $appInfo = Get-EvergreenApp -Name $appEvergreenName | Where-Object { $_.Architecture -eq $appArchitecture -and $_.Type -eq $appType -and $_.Release -eq $appRelease } |
        Sort-Object -Property @{ Expression = { [version]$_.Version }; Descending = $true } |
        Select-Object -First 1
    if ($null -eq $appInfo)
    {
    ...
    $appDesktopIconPattern = 'Wireshark*.lnk'
