...
$adtSession = @{
    # App variables.
    AppVendor = 'Mimecast'
    AppName = 'Mimecast for Outlook'
    AppVersion = '7.10.1.133'
    AppArch = 'x64'
    AppLang = 'EN'
    AppRevision = '01'
    AppSuccessExitCodes = @(0)
    AppRebootExitCodes = @(1641, 3010)
    AppProcessesToClose = @(@{ Name = 'outlook'; Description = 'Microsoft Outlook' })  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
    AppScriptVersion = '1.0.0'
    AppScriptDate = '2026-05-13'
...
function Install-ADTDeployment
{
    ...
    $appMsiInstallArgumentList = @(
        # Add MSI arguments only if you need to override PSADT defaults.
        # Examples: /qn, REBOOT=ReallySuppress, ALLUSERS=1
    )
    if ($appMsiInstallArgumentList.Count -gt 0)
    {
        Start-ADTMsiProcess -Action Install -FilePath "$($adtSession.DirFiles)\Mimecast for outlook 7.10.1.133 (x64).msi" -ArgumentList $appMsiInstallArgumentList
    }
    else
    {
        Start-ADTMsiProcess -Action Install -FilePath "$($adtSession.DirFiles)\Mimecast for outlook 7.10.1.133 (x64).msi"
    }
...
function Uninstall-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name 'Mimecast for Outlook'
