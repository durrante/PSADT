...
function Install-ADTDeployment
{
    ...
    ## Remove any existing ShareX installation before upgrading.
    $existingShareX = @(Get-ADTApplication -Name 'ShareX' -NameMatch Exact -ErrorAction SilentlyContinue)
    if ($existingShareX.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingShareX.Count) existing ShareX installation(s) - removing before upgrade."
        $preUninstallSplat = @{
            Name                   = 'ShareX'
            NameMatch              = 'Exact'
            ApplicationType        = 'EXE'
            AdditionalArgumentList = '/VERYSILENT'
        }
    ...
    $appDesktopIconPattern = 'ShareX*.lnk'
