...
$adtSession = @{
    ...
    AppProcessesToClose = @('windirstat')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
    AppScriptVersion = '1.0.0'
    AppScriptDate = '2026-06-10'
...
function Install-ADTDeployment
{
    ...
    ## Remove any existing WinDirStat installation before upgrading.
    $existingWinDirStat = @(Get-ADTApplication -Name 'WinDirStat' -NameMatch Contains -ErrorAction SilentlyContinue)
    if ($existingWinDirStat.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingWinDirStat.Count) existing WinDirStat installation(s) - removing before upgrade."
        $preUninstallSplat = @{
            Name                   = 'WinDirStat'
            NameMatch              = 'Contains'
            ApplicationType        = 'EXE'
    ...
    $appDesktopIconPattern = 'WinDirStat*.lnk'
    ...
    ## Restart Start menu process to refresh Start menu layout and pinned items.
