...
function Install-ADTDeployment
{
        ...
        Show-ADTInstallationWelcome -CloseProcesses 'WinSCP' -CloseProcessesCountdown 900 -PersistPrompt -PromptToSave
        ...
        ## Remove Any Existing Version of WinSCP (EXE)
        $AppList = Get-ADTApplication -Name 'WinSCP'    
        ...
        Start-ADTProcess -FilePath $UninstPath -ArgumentList "/VERYSILENT /NORESTART /LOG=""$(if ($isAdmin) { (Get-ADTConfig).Toolkit.LogPath } else { (Get-ADTConfig).Toolkit.LogPathNoAdminRights })\WinSCP-Uninstall.log"""
		...
		$appInfo = Get-EvergreenApp -Name $appname | Where-Object { $_.type -eq $apptype}
		$installerPath = $appInfo | Save-EvergreenApp -Path $tempPath
		
		# Install the application
		Start-Process -FilePath msiexec.exe -ArgumentList "/I `"$installerPath`" /quiet /norestart DESKTOPSHORTCUT=0" -Wait -Verbose -WindowStyle Hidden
		Sleep -Seconds 30
