...
$adtSession = @{
    ...
    AppProcessesToClose = 'Treesizefree'  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
		...
		## Show Welcome Message, close TreeSize free if required after 10 minute countdown, verify there is enough disk space to complete the install, and persist the prompt
		Show-ADTInstallationWelcome -CloseProcesses 'Treesizefree' -CheckDiskSpace -PersistPrompt -CloseProcessesCountdown 600

		## Show Progress Message (with the default message)
		## Show-InstallationProgress

		## <Perform Pre-Installation tasks here>
		$AppList = Get-ADTApplication -Name 'TreeSize'        
		...
		[string]$MyArguments = "-ExecutionPolicy Bypass -NoLogo -File `"$($adtSession.DirFiles)\Installtreesize.ps1`" "
...
function Uninstall-ADTDeployment
{
		...
		## Show Welcome Message, close Treesizefree with a 600 second countdown before automatically closing
		Show-ADTInstallationWelcome -CloseProcesses 'Treesizefree' -CloseProcessesCountdown 600
        ...
        ## Uninstall Any Existing Version of TreeSize
        $AppList = Get-ADTApplication -Name 'TreeSize'        
        ForEach ($App in $AppList)
        {
        If($App.UninstallString)
