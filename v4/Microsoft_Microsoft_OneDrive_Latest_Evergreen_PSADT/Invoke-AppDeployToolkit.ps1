...
$adtSession = @{
    ...
    AppProcessesToClose = @(@{ Name = 'OneDrive'; Description = 'Microsoft OneDrive' })  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    $appEvergreenName = 'MicrosoftOneDrive'
    $appArchitecture = 'x64'
    $appType = 'exe'
    $appRing = 'Production'
    $appThrottle = '100'
    ...
    $appInfo = Get-EvergreenApp -Name $appEvergreenName | Where-Object { $_.Architecture -eq $appArchitecture -and $_.Type -eq $appType -and $_.Ring -eq $appRing -and $_.Throttle -eq $appThrottle } |
        Sort-Object -Property @{ Expression = { [version]$_.Version }; Descending = $true } |
        Select-Object -First 1
    if ($null -eq $appInfo)
    {
    ...
    $appDesktopIconPattern = 'Microsoft OneDrive*.lnk'
