...
<#
.SYNOPSIS
    Intune detection script for Web Signer for Barclays (version >= 2.2.1.0)
.DESCRIPTION
    Searches both 64-bit and 32-bit Uninstall registry keys for an entry where DisplayName
    equals the specified application name and version >= 2.2.1.0.
    Returns exit 0 (detected) or exit 1 (not detected).
    Configure in Intune as a custom PowerShell detection script.
#>

$appDisplayName = 'Web Signer for Barclays'
$requiredVersion = [version]'2.2.1.0'
