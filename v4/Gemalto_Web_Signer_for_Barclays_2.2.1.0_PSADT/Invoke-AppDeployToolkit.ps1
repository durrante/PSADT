...
$adtSession = @{
    # App variables.
    AppVendor = 'Gemalto'
    AppName = 'Web Signer for Barclays'
    AppVersion = '2.2.1.0'
    AppArch = 'x86'
    AppLang = 'EN'
    AppRevision = '01'
    ...
    AppProcessesToClose = @(@{ Name = 'GSToolBox'; Description = 'Gemalto Classic Client Toolbox' }, @{ Name = 'bccidsystemtrayapp'; Description = 'Gemalto Bluetooth Device Manager' })  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
    AppScriptVersion = '1.0.0'
    AppScriptDate = '2026-06-09'
...
function Install-ADTDeployment
{
    ...
    ## Remove any existing Web Signer for Barclays installation before upgrading.
    $existingWebSignerforBarclays = @(Get-ADTApplication -Name 'Web Signer for Barclays' -NameMatch Exact -ErrorAction SilentlyContinue)
    if ($existingWebSignerforBarclays.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingWebSignerforBarclays.Count) existing Web Signer for Barclays installation(s) - removing before upgrade."
        $preUninstallSplat = @{
    ...
    ## Remove any existing Gemalto Bluetooth Device Manager installation before upgrading.
    $existingGemaltoBluetooth = @(Get-ADTApplication -Name 'Gemalto Bluetooth Device Manager' -NameMatch Exact -ErrorAction SilentlyContinue)
    if ($existingGemaltoBluetooth.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingGemaltoBluetooth.Count) existing Gemalto Bluetooth Device Manager installation(s) - removing before upgrade."
        $preUninstallSplat2 = @{
            Name                   = 'Gemalto Bluetooth Device Manager'
            NameMatch              = 'Exact'
            ApplicationType        = 'MSI'
        }
        Uninstall-ADTApplication @preUninstallSplat2
    }
    ...
    ## Step 1 of 2 - WebSigner_SuiteMSI.exe
    ...
    $installProcessSplat1 = @{
        FilePath       = "$($adtSession.DirFiles)\WebSigner_SuiteMSI.exe"
        WindowStyle    = 'Hidden'
        WaitForMsiExec = $true
    }
    ...
    ## Step 2 of 2 - GBDM\GTO_GBDM_Win10_With_Native_BLE_4.0.3.6.msi
    ...
    {
        Start-ADTMsiProcess -Action Install -FilePath "$($adtSession.DirFiles)\GBDM\GTO_GBDM_Win10_With_Native_BLE_4.0.3.6.msi" -ArgumentList $appMsiInstallArgumentList2
    }
    else
    {
        Start-ADTMsiProcess -Action Install -FilePath "$($adtSession.DirFiles)\GBDM\GTO_GBDM_Win10_With_Native_BLE_4.0.3.6.msi"
    }
