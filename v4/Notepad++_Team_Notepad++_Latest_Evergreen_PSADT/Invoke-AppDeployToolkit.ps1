...
function Install-ADTDeployment
{
    ...
    ## Remove any existing Notepad++ installation before upgrading.
    $existingNotepad = @(Get-ADTApplication -Name 'Notepad++' -NameMatch Contains -ErrorAction SilentlyContinue)
    if ($existingNotepad.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingNotepad.Count) existing Notepad++ installation(s) - removing before upgrade."
        $preUninstallSplat = @{
            Name                   = 'Notepad++'
            NameMatch              = 'Contains'
            ApplicationType        = 'EXE'
    ...
    $existingNotepad2 = @(Get-ADTApplication -Name 'Notepad++' -NameMatch Contains -ErrorAction SilentlyContinue)
    if ($existingNotepad2.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingNotepad2.Count) existing Notepad++ MSI installation(s) - removing before upgrade."
        $preuninstallSplat2 = @{
        Name            = 'Notepad++'
        ApplicationType = 'MSI'
    }
    ...
    $appEvergreenName = 'NotepadPlusPlus'
    $appArchitecture = 'x64'
    $appType = 'msi'
    $appInstallerType = 'Default'
    ...
    $appInfo = Get-EvergreenApp -Name $appEvergreenName | Where-Object { $_.Architecture -eq $appArchitecture -and $_.Type -eq $appType -and $_.InstallerType -eq $appInstallerType } |
        Sort-Object -Property @{ Expression = { [version]$_.Version }; Descending = $true } |
        Select-Object -First 1
    if ($null -eq $appInfo)
    {
    ...
    $appDesktopIconPattern = 'Notepad++*.lnk'
