...
$adtSession = @{
    # App variables.
    AppVendor = 'Adobe'
    AppName = 'Adobe Acrobat Pro (Unified Installer)'
    AppVersion = 'Latest'
    AppArch = 'x64'
    AppLang = 'EN'
    AppRevision = '01'
...
function Install-ADTDeployment
{
    ...
    $appEvergreenName = 'AdobeAcrobatProStdDC'
    $appArchitecture = 'x64'
    $appType = 'zip'
    $appSku = 'Pro'
    ...
    $appExeInstallArgumentList = @(
        ...
        # /quiet
        # /norestart
        '/sAll /msi /norestart /quiet ALLUSERS=1 EULA_ACCEPT=YES'
    )
    ...
    $appInfo = Get-EvergreenApp -Name $appEvergreenName | Where-Object { $_.Architecture -eq $appArchitecture -and $_.Type -eq $appType -and $_.Sku -eq $appSku } |
        Sort-Object -Property @{ Expression = { [version]$_.Version }; Descending = $true } |
        Select-Object -First 1
    if ($null -eq $appInfo)
    {
    ...
    $appDesktopIconPattern = 'Adobe*.lnk'
    ...
    ## Configure Adobe Acrobat settings
    $adobeRegBase    = 'HKLM:\SOFTWARE\Adobe\Adobe Acrobat\DC'
    $adobePolicyBase = 'HKLM:\SOFTWARE\Policies\Adobe\Adobe Acrobat\DC\FeatureLockDown'

    # Disable EULA
    Set-ADTRegistryKey -Key "$adobeRegBase\AdobeViewer"          -Name 'EULA'                              -Value 1 -Type DWord
    # Suppress "Make Adobe Acrobat my default PDF application" prompt
    Set-ADTRegistryKey -Key "$adobeRegBase\AVAlert\cCheckbox"    -Name 'iAppDoNotTakePDFOwnershipAtLaunch'       -Value 1 -Type DWord
    Set-ADTRegistryKey -Key "$adobeRegBase\AVAlert\cCheckbox"    -Name 'iAppDoNotTakePDFOwnershipAtLaunchWin10'  -Value 1 -Type DWord
    # Disable Help > Repair Installation menu
    Set-ADTRegistryKey -Key "$adobeRegBase\Installer"            -Name 'DisableMaintenance'                -Value 1 -Type DWord
    # Suppress upsell / upgrade prompts
    Set-ADTRegistryKey -Key $adobePolicyBase                     -Name 'bAcroSuppressUpsell'               -Value 1 -Type DWord
    # Disable First Time Experience (welcome tour/page)
    Set-ADTRegistryKey -Key $adobePolicyBase                     -Name 'bToggleFTE'                        -Value 1 -Type DWord
    # Disable usage measurement / feedback program
    Set-ADTRegistryKey -Key $adobePolicyBase                     -Name 'bUsageMeasurement'                 -Value 0 -Type DWord
    # Enable shared/multi-user licence mode (Acrobat or Reader depending on assigned licence)
    Set-ADTRegistryKey -Key $adobePolicyBase                     -Name 'bIsSCReducedModeEnforcedEx'        -Value 1 -Type DWord
    Set-ADTRegistryKey -Key "$adobePolicyBase\cIPM"              -Name 'bDontShowMsgWhenViewingDoc'        -Value 0 -Type DWord
...
function Uninstall-ADTDeployment
{
    ...
    Uninstall-ADTApplication -Name 'Adobe Acrobat (64-bit)'
