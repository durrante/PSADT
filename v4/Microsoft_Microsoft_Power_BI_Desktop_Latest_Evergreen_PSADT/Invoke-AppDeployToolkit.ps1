...
$adtSession = @{
    ...
    AppProcessesToClose = @('PBIDesktop')  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
    ...
    ## Remove any existing Microsoft Power BI Desktop installation before upgrading.
    ## Regex name match covers both the 'Power BI' and 'PowerBI' DisplayName variants Microsoft has shipped.
    $existingMicrosoftPowerBIDesktop = @(Get-ADTApplication -Name 'Microsoft Power ?BI Desktop' -NameMatch Regex -ErrorAction SilentlyContinue)
    if ($existingMicrosoftPowerBIDesktop.Count -gt 0)
    {
        Write-ADTLogEntry -Message "Found $($existingMicrosoftPowerBIDesktop.Count) existing Microsoft Power BI Desktop installation(s) - removing before upgrade."
        $preUninstallSplat = @{
    ...
    $appEvergreenName = 'MicrosoftPowerBI'
    $appArchitecture = 'x64'
    $appType = 'exe'
    ...
    $appDesktopIconPattern = 'Power BI Desktop*.lnk'
