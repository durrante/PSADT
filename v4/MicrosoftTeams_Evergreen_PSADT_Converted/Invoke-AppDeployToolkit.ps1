...
$adtSession = @{
    ...
    AppProcessesToClose = 'Teams'  # Example: @('excel', @{ Name = 'winword'; Description = 'Microsoft Word' })
...
function Install-ADTDeployment
{
		...
		## Show Welcome Message, close Teams if required after 10 minute countdown, verify there is enough disk space to complete the install, and persist the prompt
		Show-ADTInstallationWelcome -CloseProcesses 'Teams' -CheckDiskSpace -PersistPrompt -CloseProcessesCountdown 600
		...
		Start-ADTProcessAsUser -Path "$UninstPath" -Parameters "--uninstall -s"
		Start-Sleep -Seconds 5

		## Cleanup Microsoft Teams Application (Local User Profile) Directory
		If (Test-Path $TeamsLocal) {
		Write-ADTLogEntry -Message "Cleanup ($TeamsLocal) Directory."
		...
		Remove-ADTRegistryKey -Key 'HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\Teams' -SID $_.SID
		}
		...
		Uninstall-ADTApplication -Name 'Teams Machine-Wide Installer' -ApplicationType MSI
		...
		[string]$MyArguments = "-ExecutionPolicy Bypass -NoLogo -File `"$($adtSession.DirFiles)\InstallMSTeamsSystemWide.ps1`" "
		Start-ADTProcess -FilePath "$PSHOME\powershell.exe" -ArgumentList $MyArguments
...
function Uninstall-ADTDeployment
{
		...
		## Show Welcome Message, close Teams with a 600 second countdown before automatically closing
		Show-ADTInstallationWelcome -CloseProcesses 'Teams' -CloseProcessesCountdown 600
