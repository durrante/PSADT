...
Try {
	...
	[string]$appName = 'Zoom Workspace'
	[string]$appVersion = 'Latest'
	[string]$appArch = 'x64'
	[string]$appLang = 'mui'
	[string]$appRevision = '1.0'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '25/04/2024'
	[string]$appScriptAuthor = 'Alex Durrant'
    ##*===============================================
    ## Variables: Install Titles (Only set here to override defaults set by the toolkit)
    [String]$installName = 'Zoom Workspace'
    [String]$installTitle = 'Zoom Workspace'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$appInfo = Get-EvergreenApp -Name $appName | Where-Object { $_.Architecture -eq $appArch -and $_.Installer -eq $appInstaller -and $_.Platform -eq $appPlatform}  | `
		Sort-Object -Property @{ Expression = { [System.Version]$_.Version }; Descending = $true } | Select-Object -First 1		
		$installerPath = $appInfo | Save-EvergreenApp -Path $tempPath

		# Install Zoom
		Execute-MSI -Action 'Install' -Path "$installerPath" -Parameters '/qn'
		...
		## Remove desktop sh
		Sleep -Seconds 5
		Remove-File -Path "$envCommonDesktop\Zoom Workplace.lnk" -EA 0
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'zoom' -AllowDeferCloseApps -DeferTimes $MaxDefers -ForceCloseAppsCountdown $CloseAppsCountDownSeconds -PromptToSave -PersistPrompt -MinimizeWindows $false -TopMost $true
